...
def show_dashboard():
    st.markdown('<div class="step-card active-step"><h2>📊 Halaman Dashboard</h2><p>Selamat datang di sistem identifikasi kerusakan mobil menggunakan AI</p></div>', unsafe_allow_html=True)
    ...
    total_damages = sum(sum(c['total_damages'].values()) for c in st.session_state.processed_results) if total_processed else 0
    c3.metric("🔍 Total Damages Found", total_damages)
    heavy = sum(1 for c in st.session_state.processed_results if c['severity'][0] == 'Heavy')
    c4.metric("⚠️ Heavy Damage Cars", heavy)
    ...
    if st.session_state.processed_results:
        ...
        for car in st.session_state.processed_results[-3:]:
            ...
            st.markdown(f'<div class="processing-item severity-{sev}"><strong>🚗 {car["plate"]}</strong> — Severity: {car["severity"][0]} | Total Damages: {sum(car["total_damages"].values())}</div>', unsafe_allow_html=True)
...
def show_process_step(model):
    ...
    for i, car in enumerate(st.session_state.car_queue):
        ...
        processed.append({'plate': car['plate'], 'images': imgs, 'total_damages': totals, 'severity': severity, 'processed_time': datetime.now()})

        progress.progress((i+1)/total)
        sev_cls = severity[0].lower()
        result_box.markdown(f'<div class="processing-item severity-{sev_cls}"><strong>✅ {car["plate"]}</strong> — {severity[0]} damage | Total kerusakan: {sum(totals.values())}</div>', unsafe_allow_html=True)
...
def show_results_step():
    st.markdown('<div class="step-card active-step"><h2>📊 Tampilkan Mask Side by Side</h2><p>Hasil analisis kerusakan dengan segmentasi mask</p></div>', unsafe_allow_html=True)
        ...
        for img in car['images']:
            ...
            if sum(img['damages'].values())>0:
                ...
                for j,(k,v) in enumerate(img['damages'].items()):
                    if v>0: cols[j%6].metric(k.replace('_',' ').title(), v)
